...
def evaluate_model(model, data_iter, x_meta):
...
def train_model(name, mode, xs, ys, x_meta):
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in train_iter:
            optimizer.zero_grad()
            y_pred = model(x, x_meta, y, batches_seen)
            y_pred = scaler.inverse_transform(y_pred)
            loss = criterion(y_pred, y)
            loss_sum += loss.item() * y.shape[0]
            n += y.shape[0]
        ...
        val_loss, _ = evaluate_model(model, val_iter, x_meta)
...
def test_model(name, mode, xs, ys, x_meta):
    model = get_model(mode)
    ...
    loss, ys_pred = evaluate_model(model, test_iter, x_meta)
...
parser.add_argument('--meta_dim', type=int, default=16, help='number of meta knowledge dimension')
...
if args.dataset == 'METRLA':
    ...
    x_meta = np.load(f'../{args.dataset}/MGRU_{args.dataset}_Sembedded.npy')
    args.num_nodes = 207
...
x_meta = torch.from_numpy(x_meta).cuda(args.gpu)
print('init meta device:', x_meta.device)
...
def main():
    ...
    train_model(model_name, 'train', train_xs, train_ys, x_meta)
    ...
    test_model(model_name, 'test', test_xs, test_ys, x_meta)
