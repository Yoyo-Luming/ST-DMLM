...
def get_model(mode):
    # num_nodes, in_steps, out_steps, input_dim, hidden_dim, num_layers
    model = LSTM(num_nodes=args.num_nodes, in_steps=args.seq_len, out_steps=args.horizon, input_dim=args.input_dim, 
                    hidden_dim=args.rnn_units, num_layers=args.num_rnn_layers).to(device)
    if mode == 'train':
