...
def get_xs_ys_meta(data, data_meta, mode):
    train_num = int(data.shape[0] * args.trainval_ratio)
    xs, ys, x_meta = [], [], []
    if mode == 'train':    
        for i in range(train_num - args.horizon - args.seq_len + 1):
            x = data[i:i+args.seq_len, ...]
            y = data[i+args.seq_len:i+args.seq_len+args.horizon, ...]
            meta = data_meta[i+args.seq_len:i+args.seq_len+args.horizon, ...]
            xs.append(x), ys.append(y), x_meta.append(meta)
    elif mode == 'test':
        for i in range(train_num - args.seq_len,  data.shape[0] - args.horizon - args.seq_len + 1):
            x = data[i:i+args.seq_len, ...]
            y = data[i+args.seq_len:i+args.seq_len+args.horizon, ...]
            meta = data_meta[i+args.seq_len:i+args.seq_len+args.horizon, ...]
            xs.append(x), ys.append(y), x_meta.append(meta)
    xs, ys, x_meta = np.array(xs), np.array(ys), np.array(x_meta)
    xs, ys, x_meta = xs[:, :, :, np.newaxis], ys[:, :, :, np.newaxis], x_meta[:, :, :]
    return xs, ys, x_meta
...
def get_model(mode, adj):
    model = ST_DMLM(device=args.gpu, adj=adj, num_nodes=args.num_nodes, input_dim=args.input_dim, output_dim=args.output_dim, horizon=args.horizon, 
                    rnn_units=args.rnn_units, num_layers=args.num_rnn_layers, K=args.max_diffusion_step, meta_dim=args.meta_dim).to(device)
    if mode == 'train':
...
def train_model(name, mode, xs, ys, x_meta, adj):
    model = get_model(mode, adj)
    
    xs = scaler.transform(xs)
    xs_torch, ys_torch, meta_torch = torch.Tensor(xs).to(device), torch.Tensor(ys).to(device), torch.Tensor(x_meta).to(device)
    trainval_data = torch.utils.data.TensorDataset(xs_torch, ys_torch, meta_torch)
...
def test_model(name, mode, xs, ys, x_meta, adj):
    model = get_model(mode, adj)
    model.load_state_dict(torch.load(modelpt_path))
    
    xs = scaler.transform(xs)
    xs_torch, ys_torch, meta_torch = torch.Tensor(xs).to(device), torch.Tensor(ys).to(device), torch.Tensor(x_meta).to(device)
    test_data = torch.utils.data.TensorDataset(xs_torch, ys_torch, meta_torch)
...
parser.add_argument('--meta_type', type=str, default='ST', help='the input of meta knowledge type')
parser.add_argument('--meta_dim', type=int, default=419, help='the dimension of meta input')
...
if args.dataset == 'METRLA':
    ...
    data_path = f'../{args.dataset}/metr-la.h5'
    meta_path = f'../{args.dataset}/METRLA_{args.meta_type}embedded.npy'
    adj_path = f'../{args.dataset}/adj_mx.pkl'
    adj_file_type = "pkl"
    ...
    meta_path = f'../{args.dataset}/PEMSBAY_{args.meta_type}embedded.npy'
...
# model_name = f'{args.meta_type}_DMLM'
model_name = 'ST_DMLM'
timestring = time.strftime('%Y%m%d%H%M%S', time.localtime())
path = f'../save/{args.dataset}_{model_name}_{args.meta_type}_{timestring}'
...
data_meta = np.load(meta_path)
print('data shape:',data.shape, 'data meta shape', data_meta.shape)
...
def main():
    ...
    train_xs, train_ys, train_x_meta = get_xs_ys_meta(data, data_meta, 'train')
    logger.info('Train xs.shape ys.shape, x_meta.shape', train_xs.shape, train_ys.shape, train_x_meta.shape)
    train_model(model_name, 'train', train_xs, train_ys, train_x_meta, adj)
    logger.info(args.dataset, 'training ended', time.ctime())
    
    test_xs, test_ys, test_x_meta= get_xs_ys_meta(data, data_meta, 'test')
    logger.info('Test xs.shape, ys.shape, x_meta.shape', test_xs.shape, test_ys.shape, test_x_meta.shape)
    test_model(model_name, 'test', test_xs, test_ys, test_x_meta, adj)
